...
def main(args):
	...
	if args.visualize_all:
		...
		plotCells(args.output_folder+"1.png",plotParams, cellList)
	...
	if args.visualize_all:
		...
		plotCellClusters(args.output_folder+"2.png",plotParams, cellClusters, spillRadiusImg)
		#Plot clusters and all transcripts
		plotCellClusters(args.output_folder+"3.png",plotParams, cellClusters, spillRadiusImg, t_e_crds)
	...
	newJson = args.output_folder+args.json[start_slice:-5]+"_cells.json"
	...
	writeCellClusters(args.output_folder+"cell_clusters.tsv", cellClusters)

	#Plot final visualization of the clustering + transcript assignment process
	if args.visualize or args.visualize_all:
		plotFinalImage(args.output_folder+"4.png", plotParams,cellClusters, feat_in_clusters,spillRadiusImg, st)
		if args.counts:
			plotFinalImage(args.output_folder+"4_counts.png", plotParams,cellClusters, feat_in_clusters,spillRadiusImg, st,"counts", unique_events)
		if args.unique_genes:
			plotFinalImage(args.output_folder+"4_uniquegenes.png", plotParams,cellClusters, feat_in_clusters,spillRadiusImg, st,"unique_genes", unique_events)
...
def plotFinalImage(filename,plotParams,cellClusters, feat_in_clusters, spillRadiusImg, transform,annot=None, unique_events=None):
	clusterColors = itertools.cycle(["red","green", "magenta", "yellow","orange","purple","blue"])
...
if __name__ == '__main__':
	...
	parser.add_argument("json",help="JSON database file with gene expression levels for each barcode")
	parser.add_argument("transform",help=".npy file with the appropiate coordinate transform between the image and the chip design")
	parser.add_argument("cells",help=".tsv file with the enclosing cells as detected by the ST plugin for FIJI/ImageJ")
	
	
	parser.add_argument("-d","--diffusion", help="Define the expected transcript diffusion radius (in number of features). Default: 2",default=2)
	parser.add_argument("-o","--output-folder", help="Folder where to output the results", default="./")
	parser.add_argument("--dpi", help="Output image DPI. This should match the original images' DPI", default=72)
	
	groupV = parser.add_mutually_exclusive_group()
	groupV.add_argument("-v","--visualize", help="Overlay the result of clustering and data integration on top of the provided image ",metavar="tissue_image_file", default=None)
	groupV.add_argument("-V","--visualize-all",help="Create overlay images of the cell clustering and transcript assignment process and result",metavar="tissue_image_file",default=None)
	#Visualization extras					
	parser.add_argument("-u","--unique-genes",action="store_true", help="Plot additional final image annotating each feature with the number of unique genes it contains. Requires -v or -V.")
	parser.add_argument("-c","--counts",action="store_true", help="Plot additional final image annotating each feature with the number of reads it contatins. Requires -v or -V.")
